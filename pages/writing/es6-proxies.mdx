...
Imagine you had an array that needed to notify part of your application when a
new value was pushed into it:
...
However ES6 Proxies actually provide a native solution that doesn't rely on
overriding the method, but enables you to intercept `all` methods and accessors
as you see fit. We could re-implement the above as a Proxy like so:
...
While there are a variety of [interesting
workarounds](http://exploringjs.com/es6/ch_classes.html#sec_private-data-for-classes)
for storing private data and methods on classes, a Proxy is a neat solution. As
long as all private properties are prefixed with an underscore (or consistent
delimiter), it's possible to intercept method calls and refuse access.
...
While this prevents their use, it doesn't stop people inspecting the source
class by logging out `MyClass` and looking at the original handler, which will
contain all the prefixed methods, and so is hardly 'privacy'. Nevertheless, it
does demonstrate how powerful proxies can be in intercepting calls.
