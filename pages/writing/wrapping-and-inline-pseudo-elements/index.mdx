...
import exampleImageMd from './example-md.jpg';
import exampleImageLg from './example-lg.jpg';
import awkwardWrappingImageMd from './awkward-wrapping-md.jpg';
import awkwardWrappingImageLg from './awkward-wrapping-lg.jpg';
import previewImage from './text-wrapping-og.png';
...
<Image 
  ...
  height={640}
  src={headerImage}
  alt="Screenshot of text containing a link, followed by an icon that indicates it's external."
/>
...
<a href="https://github.com/ampaze" target="_blank" rel="noopener noreferrer">Tim</a> Baumgarten has pointed out that the above mask technique can be [buggy in Safari](https://jsfiddle.net/34rmz5k7/2/), resulting in a background 'bleed' from the top/bottom due to line-height. 

Tim's fix is to composite two masks together ⁠— one to hide the background color in its entirety (using a fully black gradient), and one to 'cut' a hole in the first (using SVG). It's an easy and super elegant solution that seems to work well in all the mask-supporting browsers I've tested.
